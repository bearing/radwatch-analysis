...
def plot_peaks(spectrum, title= None, energy_range= None, subregion='both', use='NAA'):
    """
    ...
    if subregion == 'both':
        ...
        for i in range(int(len(center_peak_region)/2)):
            ...
            if energy_axis[center_peak_region[2*i]] <= 604.72 <= energy_axis[center_peak_region[2*i+1]]:
                flag = True
...
def test7():
    measurement = SPEFile.SPEFile('UCB027_Sea_Salt.Spe')
    measurement.read()
    plot_peaks(measurement, 'UCB027_Sea_Salt', subregion='NAA', use='NAA')
