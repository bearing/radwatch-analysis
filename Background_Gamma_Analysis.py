#################################################
#Background_Gamma_Analysis.py 
#This code is designed primarily to measure activities of backgrounds in Bq. 
#It is ran the same as way the regular Gamma_Analysis.py is. 
#When entering the background spectrums you want to measure into Radwatch_Samples.csv, just put in an arbitrary mass. It will produce the same results regardless of mass.  
#-TAB
#################################################
...
def make_table(isotope_list, sample_info, sample_names, dates):
    ...
    for i in range(len(isotope_list)):
        isotope_act_unc.append(str(isotope_list[i].symbol) + '-' +
                               ...
                               ' Act' + '[Bq]')  #Usually Bq/Kg
        isotope_act_unc.append(str(isotope_list[i].symbol) + '-' +
                               str(isotope_list[i].mass_number) +
                               ' Unc' + '[Bq]') #Usually Bq/Kg
...
def analyze_isotope(measurement, isotope): #removed background from second argument. TB Removed reference from third argument
    """
    ...
    for j, energy in enumerate(isotope_energy):
        ...
        print('Measurement Livetime (min)')
        print(measurement.livetime/60)
        print('Area, Uncertainity, energy') 
        print(sample_net_area, energy) 
        print('cpm, cpm uncertainity') 
        print(sample_net_area[0]/measurement.livetime*60, sample_net_area[1]/measurement.livetime*60)

        peak_emission = emission_rate(sample_net_area, isotope_efficiency[j],       
...
def analyze_spectrum(measurement): 
    """
    ...
    for isotope in isotope_list:
        info = analyze_isotope(measurement, isotope)
...
def main():
    ...
    for sample in sample_measurements:
        ...
        data = analyze_spectrum(measurement)
