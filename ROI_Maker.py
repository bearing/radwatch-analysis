...
def ROI_Maker(spectra, use='NAA'):
    ...
    if use == 'NAA':
        for i in range(len(peak_channels)):
            peak_energies.append(energy_per_channel * peak_channels[i] +
                                 zero_offset)
    ...
    if use == 'GA':
        ...
        for i in range(len(peak_energies_original)):
            peak_energies.append(Gamma_Analysis.peak_finder(spectra,
                                 peak_energies_original[i]))

        peak_channels = []

        for i in range(len(peak_energies)):
            peak_channels.append((peak_energies[i] - zero_offset) /
                                 energy_per_channel)

    for i in range(len(peak_energies)):
        fwhm = 0.05 * peak_energies[i]**0.5
        fwhm_channel = int(region_size * (fwhm-zero_offset) /
                           energy_per_channel)
